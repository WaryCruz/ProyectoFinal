...
<?php foreach(glob($plugDir.'/*/plugin.php') as $plugin)
	{
		...
		if(file_exists($plugin)) {
			...
			echo "<tr>";
			echo "<td><h4>".$app["Name"]."</h4>";
			if(!in_array(get_folder_name($plugin),$active)) {
			echo '<p><a class="btn btn-sm btn-outline btn-primary" href="'.admin_url('plugins').'&activate='.get_folder_name($plugin).'">Activate </a></p>';
			} else {
			echo '<p><a class="btn btn-sm btn-outline btn-info" href="'.admin_url('plugins').'&disable='.get_folder_name($plugin).'">Disable</a></p>';	
			}	
			echo "</td>";
			echo "<td>";
			...
			echo "<td>";
			if(!in_array(get_folder_name($plugin),$active)) {
			echo '<span class="badge badge-radius badge-warning">Disabled</span>';
			} else {
			echo '<span class="badge badge-radius badge-success">Active</span>';	
			}
			echo "</td>";
			echo "</tr>";
